...
function Card({ title, file, creatorUsername, id, creator, likes, likesArrayItem }) {
  ...
  const isUserLiked = likesArrayItem.includes(ctx.userId);
  ...
  return (
    <div className='card'>
      {responseMessage ? (
        ...
        <>
          <h2 style={{ alignSelf: 'flex-start', marginBottom: '5px', cursor: 'pointer' }} onClick={handleProfile}>
            {creatorUsername}
          </h2>
          ...
          <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
            <h3 onClick={likeHandler} style={{ marginRight: '10px', cursor: 'pointer' }}>
              {isUserLiked ? <AiFillHeart /> : <AiOutlineHeart />}
            </h3>
            <h4>{likes} LIKES</h4>
          </div>

          <div className='cardFooter'>
            <h3 style={{ cursor: 'pointer' }} onClick={handleProfile}>
              {creatorUsername}
            </h3>
            <h6 style={{ marginLeft: '10px' }}> {title}</h6>
          </div>

          {creator === ctx.userId && <button type='button' className='btn-danger' onClick={handleDelete}>
            DELETE
          </button>}
        </>
      )}
